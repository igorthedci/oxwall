...
class OxwallSite:
    ...
    def login_as(self, user):
        ...
        driver = self.driver
        driver.find_element(*newsfeed_page.SIGN_IN_MENU).click()
        login = driver.find_element(*newsfeed_page.LOGIN_FIELD)
        login.click()
        login.send_keys(user.username)
        passw = driver.find_element(*newsfeed_page.PASS_FIELD)
        passw.click()
        passw.send_keys(user.password)
        driver.find_element(*newsfeed_page.SIGN_IN_BUTTON).click()
        # Wait until grey background disappeared
        wait = WebDriverWait(driver, 5)
        wait.until(EC.invisibility_of_element_located(newsfeed_page.LOGIN_BACKGROUND))
